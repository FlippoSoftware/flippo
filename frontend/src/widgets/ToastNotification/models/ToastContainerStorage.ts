...
function composeNotification(variant: TNotificationKind, { statusIconSize, timeout, ...otherPayload }: TToastCreate): TToastStore {
  const id = createUUID();
  return {
    id,
    statusIconSize: statusIconSize || defaultStatusIconSize[variant],
    timeout: timeout || defaultTimeout[variant],
    variant,
    ...otherPayload
  };
}
