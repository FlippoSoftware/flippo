...
import { displayRequestError, displayRequestSuccess } from '@widgets/ToastNotification';
...
countDuplicateMut.$error.watch(data => console.error(data));
...
createFolderMut.start.watch(() => console.warn('CREATE FOLDER'));
createFolderMut.finished.success.watch(() => console.warn('SUCCESS'));
createFolderMut.finished.failure.watch(() => console.error('FAIL'));
...
countDuplicateMut.$error.watch(e => console.error(e));
createFolderMut.finished.failure.watch(({ error }) => console.error(error));
