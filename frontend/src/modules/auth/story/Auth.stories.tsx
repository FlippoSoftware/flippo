...
import Auth from '../view/ui/Auth/Auth';
...
function WithCode(props: { checking?: boolean; email: string; error?: null | string }) {
  ...
  const [onVerificationCodeErrorChanged, onTimerStart] = useUnit([verificationCodeErrorChanged, timerStart]);
  ...
  const errorBind = scopeBind(onVerificationCodeErrorChanged, { scope: scopeStory });
  const timerBind = scopeBind(onTimerStart, { scope: scopeStory });
