...
export function createStorage<T>(type: StorageType, hookName: string) {
    ...
    return function useStorage({
        ...
        const readStorageValue = useStableCallback(
        ...
        const [value, setValue] = React.useState<T>(readStorageValue(getInitialValueInEffect));

        const setStorageValue = React.useCallback(
            (val: T | ((prevState: T) => T)) => {
                if (typeof val === 'function') {
                    const updater = val as (prevState: T) => T;
                    setValue((current) => {
                        ...
                        queueMicrotask(() => {
                            window.dispatchEvent(
                                new CustomEvent(eventName, { detail: { key, value: updater(current) } })
                            );
                        });
                        return result;
                    });
