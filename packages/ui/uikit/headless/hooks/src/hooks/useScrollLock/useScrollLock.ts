...
function preventScrollInsetScrollbars(referenceElement: Element | null) {
    ...
    function lockScroll() {
        ...
        const marginY = Number.parseFloat(bodyStyles.marginTop) + Number.parseFloat(bodyStyles.marginBottom);
        const marginX = Number.parseFloat(bodyStyles.marginLeft) + Number.parseFloat(bodyStyles.marginRight);
        const elementToLock = isOverflowElement(html) ? html : body;
