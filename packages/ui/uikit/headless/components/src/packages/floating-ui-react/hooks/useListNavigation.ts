...
export function useListNavigation(
    ...
    const getParentOrientation = React.useCallback(() => {
        return (
            parentOrientation
            ?? (tree?.nodesRef.current.find((node) => node.id === parentId)?.context?.dataRef?.current.orientation as UseListNavigationProps['orientation'])
        );
            ...
            getGridNavigatedIndex(
                {
                    current: cellMap.map((itemIndex) =>
                        itemIndex != null ? listRef.current[itemIndex] ?? null : null
                    )
                },
                {
                    event,
