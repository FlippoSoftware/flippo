...
const DEFAULT_ORDER = ['content'] as NonNullable<FloatingFocusManagerProps['order']>;
...
export function FloatingFocusManager(props: FloatingFocusManagerProps): React.JSX.Element {
    ...
    const mergedBeforeGuardRef = useMergedRef(
        beforeGuardRef,
        beforeContentFocusGuardRef,
        ...
        <React.Fragment>
                ...
                <FocusGuard
                    ...
                    onFocus={(event) => {
                        if (modal) {
                            ...
                            enqueueFocus(els[els.length - 1] ?? null);
                        }
            ...
            {shouldRenderGuards && (
                <FocusGuard
                    ...
                    onFocus={(event) => {
                        if (modal) {
                            enqueueFocus(getTabbableElements()[0] ?? null);
                        }
                        else if (portalContext?.portalNode) {
                            if (closeOnFocusOut) {
                                preventReturnFocusRef.current = true;
