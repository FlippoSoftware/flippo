...
import { useOnFirstRender } from '@flippo-ui/hooks/use-on-first-render';
import { useOpenChangeComplete } from '@flippo-ui/hooks/use-open-change-complete';
import { useOpenInteractionType } from '@flippo-ui/hooks/use-open-interaction-type';
import { useStableCallback } from '@flippo-ui/hooks/use-stable-callback';
import { Store, useStore } from '@flippo-ui/hooks/use-store';
...
} from '~@lib/createHeadlessUIEventDetails';
import {
    defaultItemEquality,
    findItemIndex,
    itemIncludes,
    removeItem
} from '~@lib/itemEquality';
...
import type { Group } from '~@lib/resolveValueLabel';
import type { HTMLProps } from '~@lib/types';
import type { ElementProps } from '~@packages/floating-ui-react';

import { useFieldRootContext } from '../../Field/root/FieldRootContext';
import { useField } from '../../Field/useField';
import { useFormContext } from '../../Form/FormContext';
import { useLabelableId } from '../../LabelableProvider/useLabelableId';
...
export function AriaCombobox<Value = any, Mode extends SelectionMode = 'none'>(
    ...
    const [selectedValue, setSelectedValueUnwrapped] = useControlledState<any>({
        prop: selectedValueProp,
        defaultProp: multiple ? (defaultSelectedValue ?? EMPTY_ARRAY) : defaultSelectedValue,
        caller: 'Combobox'
    });
    ...
    const initialDefaultInputValue = useLazyRef<React.ComponentProps<'input'>['defaultValue']>(
        () => {
            if (hasInputValue) {
    ...
    const [inputValue, setInputValueUnwrapped] = useControlledState({
        prop: inputValueProp,
        defaultProp: initialDefaultInputValue,
        caller: 'Combobox'
    });
    ...
    const { getReferenceProps, getFloatingProps, getItemProps } = useInteractions([role, click, dismiss, listNavigation]);
    ...
    const hiddenInputRef = useMergedRef(inputRefProp, validation.inputRef);

    const itemsContextValue: ComboboxDerivedItemsContextValue = React.useMemo(
        () => ({
            query,
            filteredItems,
            flatFilteredItems
...
type ComboboxRootProps<ItemValue> = {
     ...
     * Receives the highlighted item value (or `undefined` if no item is highlighted) and event details
     * with a `reason` property describing why the highlight changed.
     * The `reason` can be:
     ...
     * If the shape of the object is `{ value, label }`, the label will be used automatically without needing
     * to specify this prop.
     */
    itemToStringLabel?: (itemValue: ItemValue) => string;
    /**
     * When the item values are objects (`<Combobox.Item value={object}>`), this function converts the object
     * value to a string representation for form submission.
     * If the shape of the object is `{ value, label }`, the value will be used automatically without needing
     * to specify this prop.
     */
     ...
     *   on outside elements are disabled.
     ...
     * - `list` (default): items are dynamically filtered based on the input value. The input value does not change
     *   based on the active item.
     * - `both`: items are dynamically filtered based on the input value, which will temporarily change based on the
     *   active item (inline autocompletion).
     * - `inline`: items are static (not filtered), and the input value will temporarily change based on the active
...
export namespace AriaCombobox {
    ...
    export type HighlightEventDetails = HeadlessUIGenericEventDetails<
