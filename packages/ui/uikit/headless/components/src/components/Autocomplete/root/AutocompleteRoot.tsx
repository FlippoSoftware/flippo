...
import { stringifyAsLabel } from '~@lib/resolveValueLabel';
...
export type AutocompleteRootActions = {
    unmount: () => void;
};
...
export type AutocompleteRootProps<ItemValue> = {
     ...
     * - `inline`: items are static (not filtered), and the input value will temporarily change based on the active item
    ...
    /**
     * When the item values are objects (`<Autocomplete.Item value={object}>`), this function converts the object value
     * to a string representation for both display in the input and form submission.
     ...
     * Receives the highlighted item value (or `undefined` if no item is highlighted) and event details with a `reason`
     * property describing why the highlight changed.
     * The `reason` can be:
