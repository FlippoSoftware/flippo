...
export function useSelectRoot<Value, Multiple extends boolean | undefined>(
    ...
    const [value, setValueUnwrapped] = useControlledState({
        prop: params.value,
        defaultProp: multiple ? (params.defaultValue ?? EMPTY_ARRAY) : params.defaultValue,
        caller: 'Select'
    });

    const [open, setOpenUnwrapped] = useControlledState({
        prop: params.open,
        defaultProp: Boolean(params.defaultOpen),
        caller: 'Select'
    });
    ...
    useIsoLayoutEffect(() => {
        ...
        if (multiple) {
            ...
            const lastIndex = findItemIndex(valuesRef.current, lastValue as Value, isItemEqualToValue);
    ...
    const floatingContext = useFloatingRootContext({
        open,
        onOpenChange: setOpen as (open: boolean, eventDetails: HeadlessUIChangeEventDetails<string>) => void,
        elements: {
