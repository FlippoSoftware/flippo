...
import type { HeadlessUIEvent, WithHeadlessUIEvent } from './types';
...
type PropsOf<T extends React.ElementType> = WithHeadlessUIEvent<React.ComponentPropsWithRef<T>>;
...
function mergeEventHandlers(ourHandler: Function, theirHandler: Function) {
    ...
    return (event: unknown) => {
        if (isSyntheticEvent(event)) {
            const headlessUIEvent = event as HeadlessUIEvent<typeof event>;

            makeEventPreventable(headlessUIEvent);

            const result = theirHandler(headlessUIEvent);

            if (!headlessUIEvent.headlessUIHandlerPrevented) {
                ourHandler?.(headlessUIEvent);
            }
...
export function makeEventPreventable<T extends React.SyntheticEvent>(event: HeadlessUIEvent<T>) {
    event.preventHeadlessUIHandler = () => {
        (event.headlessUIHandlerPrevented as boolean) = true;
    };
