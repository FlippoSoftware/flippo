...
import { useAnimationFrame } from '@flippo-ui/hooks/use-animation-frame';
            ...
            if (entry) {
                if (previousDimensionsRef.current === null) {
                    previousDimensionsRef.current = {
                        width: Math.ceil(entry.borderBoxSize[0]?.inlineSize ?? 0),
                        height: Math.ceil(entry.borderBoxSize[0]?.blockSize ?? 0)
                    };
                }
                else {
                    previousDimensionsRef.current.width = Math.ceil(entry.borderBoxSize[0]?.inlineSize ?? 0);
                    previousDimensionsRef.current.height = Math.ceil(entry.borderBoxSize[0]?.blockSize ?? 0);
                }
            }
        });
...
type UsePopupAutoResizeParameters = {
    /**
     * Element to resize.
     */
